...
public class CpClientExampleStockholm {
	...
	public static void main(String[] args) { 
		...
		try {
			...
			CpRouteRequest cprr1 = cgi.getCityRoutes(18.003, 59.2732, 18.0525, 59.3687, CpRouteRequest.ROUTE_COST_METRIC_TIME, 1);
//			System.out.println("cprr1:\n"+cprr1);
			int distance = cprr1.getFirstRoute().getLengthM();
			int time = cprr1.getFirstRoute().getTimeS();
			System.out.println(distance + "m, " + time + "s. Processing output...");
			String waypointsStr = cprr1.getFirstRoute().getGeom().getGeometry().getValue();
			String[] list = waypointsStr.substring(1,waypointsStr.length()-1).split(",");
